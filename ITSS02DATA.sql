...
select assets.id,assetsn, assetname, departments.name
	...
	select * from emergencymaintenances
		...
		select a.id,assetsn,assetname,emenddate,(select count(*) from emergencymaintenances where assetid=em.assetid ) as numem from assets a,emergencymaintenances em where a.id=em.assetid and emenddate =(select max(emenddate) from emergencymaintenances where assetid=a.id) order by assetid asc